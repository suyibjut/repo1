...
# The original file consists of 90,000 lines, divided into 100 groups, 
# with each group containing 900 lines, corresponding to 900 warc files.
...
def wait_for_space(path, required_space, timeout=30):
    ...
    while duraiton < timeout :
        ...
        if free_space >= required_space:
            ...
            print(f"waiting for disk space: {free_space / 1024**3} GB")
            time.sleep(10)  
            duraiton += 10
    return False     
...
def download_warc(warc_table, chunk, volume, warc_dir:Path, rnew=False):
    ...
    try:       
        ...
        logger.debug(f"Gzip download to: {gzip_file_path}")        
...
def unzip_warc(chunk, volume, gzip_file_path, warc_dir:Path):
    ...
    logger.debug(f"download to: {warc_path}")  
    return  warc_path
    ...
    if not warc_table_path.exists():
        ...
        with gzip.open(gz_path, 'rb') as gz_file:
            with open(warc_table_path, 'wb') as dest_file:
                shutil.copyfileobj(gz_file, dest_file)
        logger.debug(f"warc table is downloaded into: {warc_table_path}")
        os.remove(gz_path)   
...
if __name__ == "__main__":
    tq = tqdm()
    download_and_extract(0,0, Path("/xx/xx/projects/UICoder/data"), tq)
