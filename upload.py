...
chunk_idx = '10'
path = f'/xx/xx/xx/code/UICoder/datasets/c4-wash/scored-chunks/chunk{chunk_idx}-format-scored-parquet'
score_limit = 2
...
# The dataset is restructured to include three components: training, testing, and evaluation.
...
while i<len(score_list):
    try:
        ...
        ds.push_to_hub('xcodemind/cc_scored',token="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", data_dir=f"chunk{chunk_idx}/Score[{score_list[i]}]")
        i+=1
    except:
        continue
...
#     volume_path = os.path.join(path, f'{i}')
    ...
    #         ds.push_to_hub('xcodemind/cc_scored',token="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", data_dir=f"chunk{chunk_idx}/Score[{i}]/{os.path.splitext(filename)[0]}")
    #         j+=1
    #     except:
    #         time.sleep(10)
    #         continue
...
# ds_all.push_to_hub('xcodemind/cc_scored',token="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", data_dir="chunk05")